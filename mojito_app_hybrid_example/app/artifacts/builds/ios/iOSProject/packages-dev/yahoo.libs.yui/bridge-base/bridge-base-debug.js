        ...
        if ((!contextId && this._isContextIdResponse(message)) || (message.d && message.d[0] === contextId)) {
            ...
            if (callbacks) {
                ...
                if (!(message.p && message.p.c === false)) {
                    Y.log("Got cancelOnCB. Removing callbackMap entry for invocationId: " + invocationId, "info", "bridge-base");
                ...
                else { Y.log("Unknown callback function key: " + Y.JSON.stringify(message), "error", "bridge-base"); }
            }
            else { Y.log("No registered callbacks for this invocationId: " + invocationId, "warn", "bridge-base"); }
        }
        else { Y.log("Missing Destination: " + Y.JSON.stringify(message), "error", "bridge-base"); }
    },
    ...
    _storeCallbacks : function(invocationId, callbacks) {
        ...
        if (callbacks && invocationId) {
            if (cbMap[invocationId]) {
                Y.log("Trying to register callbacks with an invocation id which has already been used", "error", "bridge-base");
            } else {
                cbArray = [];
        ...
        /*jshint maxlen:1000 */
        else { Y.log("Not storing callbacks. Either invocationId is not defined, or callbacks are not defined. invocationId: " + invocationId, "warn", "bridge-base"); }
        ...
        if (!YUI.Env._BridgeContext[type]) {
            ...
            YUI.Env._BridgeContext[type] = {
                ...
                sendResponses : function(jsonMessages) {
                    ...
                    else { Y.log("Unexpected callback JSON format: " + jsonMessages, "error", "bridge-base"); }
                }
            };
        }
