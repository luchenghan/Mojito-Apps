...
// Log Delegate
//
// Listeners can listen for RTLog events but cannot be queried. The events should never be handled inside a syncBlock,
// instead they must be processed inside an asyncBlock.
//
...
//
- (BOOL)setSeverity:(RTLogSeverity)severity
...
// and adjust severity. All calls are synchornous to ensure consistency and so the log messages are inline with actual
// event sources.
//
...
// Lookup Component
//
- (id <RTLogComponent>)componentWithName:(NSString *)name;
...
// Register Component
//
#define RTRegisterComponentCompSev(__comp__, __sev__) \
    do { \
        [[(__comp__) logger] registerComponent:(__comp__) \
...
//
#define RTRegisterComponentComp(__comp__) \
    RTRegisterComponentCompSev(__comp__, RTLOG_DEFAULT_SEVERITY)
...
#define RTRegisterComponent() \
    RTRegisterComponentComp(self)

//
// Regsiter self w/ Specific Severity Levels
//
#ifdef DEBUG
#define RTRegisterComponentSev2(__debug_sev__, __release_sev__) \
    RTRegisterComponentLoggerSev([self logger], __debug_sev__)

#define RTRegisterComponentCompSev2(__comp__, __debug_sev__, __release_sev__) \
    RTRegisterComponentCompSev((__comp__), __debug_sev__)
...
#define RTRegisterComponentCompSev2(__comp__, __debug_sev__, __release_sev__) \
    RTRegisterComponentCompSev((__comp__), __release_sev__)
#endif
