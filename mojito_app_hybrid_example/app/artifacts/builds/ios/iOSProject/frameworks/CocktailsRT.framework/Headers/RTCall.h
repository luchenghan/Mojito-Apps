...
// Callback block with NSString* callback, NSDictionary* args, NSDictionary* ychromertOptions, void(^completion)())
typedef void (^RTCallCallbackBlock)(NSString *, NSDictionary *, NSDictionary*, void(^)());
...
//
// The concatenated value of contextID and handleID
//
...
//
@property (nonatomic, copy) RTCallCallbackBlock callbackBlock;
...
- (id)initWithID:(NSString *)callID contextID:(NSString *)contextID handleID:(NSString *)handleID
         ...
         options: (NSDictionary *)options callbackBlock:(RTCallCallbackBlock)callbackBlock;
...
// Reply "failure" to the call with the given error and call completionBlock ONLY if the failure was sent.
- (BOOL)endCallWithError:(RTError *)error completion:(void(^)())completionBlock;
