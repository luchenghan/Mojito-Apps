...
//
// Use this property to obtain this RTObject's NSOperationQueue for use with these special iOS asynchronous APIs.
//
...
// Current RTObject
//
// Return the current RTObject that we are running on behalf of
//
+ (RTObject *)currentObject;
...
//
// Asynchronously Invoke Block On Main Thread (queue this block and return, block executes when its turn arrives).
//
- (void)mainBlock:(void (^)())block;
...
//
// Create RTTimer w/ Timeout (milliseconds) and Callback
//
// Remember to resume timer when ready.
//
